...
def issue_books(home,issue_book):
    ...
    Button(issue_book,text='Submit',command=lambda:issue(book_id,Student_id),bg='black', fg='white', font=('Courier',25)).pack(pady=12)
    home.pack_forget()
def issue(book_id,Student_id):
    ...
    try:
        ...
        query_issue="insert into Issue values("+str(issue_id)+","+Student_id+","+book_id+",'"+issue_date+"','"+return_date+"',NULL)"
        connect(query_issue) 
